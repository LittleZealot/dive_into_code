...
def visualize_descent(df, points_list):
    plt.figure(figsize=(10, 6))
    plt.plot(df["x"], df["elevation"], label="Mt. Fuji Elevation", linestyle='-', color='blue')
    plt.scatter(points_list, df.loc[points_list, "elevation"], color='red', label="Descent Points", marker='o')
    plt.title("Mt. Fuji")
    plt.xlabel("Position")
...
starting_point = 136
descended_points = descend_mountain(starting_point, df)
visualize_descent(df, descended_points)
